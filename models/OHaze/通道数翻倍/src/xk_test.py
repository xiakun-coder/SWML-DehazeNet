...
best_model_path = '../models/SOTS/best_model.pth'
...
validation_original_data_path = script_dir / '..' / 'train_input' / 'Validation'
...
for epoch in range(NUM_EPOCHS):
    ...
    MAX_SAVE = 16  # 只存 16 张 whole image
...
with torch.no_grad():
    ...
    for i, batch in enumerate(validation_patch_data_loader):
        ...
        validation_batch_patch_ssim += piq.ssim(
            outputs, targets, data_range=1., reduction='mean'
        ).item()
        validation_batch_patch_mse += torch.nn.functional.mse_loss(
            outputs, targets
        ).item()

    validation_patch_loss.append(
        validation_batch_patch_loss / len(validation_patch_data_loader)
    )
    validation_patch_psnr.append(
        validation_batch_patch_psnr / len(validation_patch_data_loader)
    )
    validation_patch_ssim.append(
        validation_batch_patch_ssim / len(validation_patch_data_loader)
    )
    validation_patch_mse.append(
        validation_batch_patch_mse / len(validation_patch_data_loader)
    )
    ...
    for i, batch in enumerate(validation_original_data_loader):
        ...
        # ===== 只保存前 MAX_SAVE 张 =====
        if len(validation_whole_gen_images) < MAX_SAVE:
            validation_whole_gen_images.append(outputs.squeeze(0).cpu())
            validation_whole_gt_images.append(targets.squeeze(0).cpu())
            validation_whole_haze_images.append(inputs.squeeze(0).cpu())

        validation_batch_whole_psnr += piq.psnr(outputs, targets).item()
        validation_batch_whole_ssim += piq.ssim(
            outputs, targets, data_range=1., reduction='mean'
        ).item()
        validation_batch_whole_mse += torch.nn.functional.mse_loss(
            outputs, targets
        ).item()

    validation_whole_loss.append(
        validation_batch_whole_loss / len(validation_original_data_loader)
    )
    validation_whole_psnr.append(
        validation_batch_whole_psnr / len(validation_original_data_loader)
    )
    validation_whole_ssim.append(
        validation_batch_whole_ssim / len(validation_original_data_loader)
    )
    validation_whole_mse.append(
        validation_batch_whole_mse / len(validation_original_data_loader)
    )

# ================= Saving Validation Images =================
# 最多 MAX_SAVE 张，不足就用实际数量
num_images = len(validation_whole_gen_images)
...
if num_images > 0:
    ...
    current_val_psnr = validation_whole_psnr[-1]  # 当前epoch的验证整体损失
