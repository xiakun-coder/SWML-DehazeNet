...
best_model_path = '../models/DenseHaze/6normalize.pth'
...
#-----------------  denseHaze数据集 -----------------
def loss_FFT_single(pred_img, label_img, eps=1e-8, clip_val=1e4):
    pred_fft = torch.fft.fft2(pred_img, dim=(-2, -1))
    label_fft = torch.fft.fft2(label_img, dim=(-2, -1))
    ...
    # only clip extreme values (Dense-Haze protection)
    pred_fft = torch.clamp(pred_fft, -clip_val, clip_val)
    label_fft = torch.clamp(label_fft, -clip_val, clip_val)
...
for epoch in range(NUM_EPOCHS):
    ...
    with torch.no_grad():
        ...
        # Whole image validation (只取前 8 张用于可视化)
        for i, batch in enumerate(validation_original_data_loader):
            ...
            # 只保存前 8 张图像用于拼接
            if len(validation_whole_gen_images) < 8:
    ...
    # Saving the Validation Images Generated (只拼接 8 张)
    images = torch.cat([
    ...
    if epoch == 100:
        torch.save(model.state_dict(), './models/dehazing_model_100.pth')

    if epoch == 120:
        torch.save(model.state_dict(), './models/dehazing_model_120.pth')
